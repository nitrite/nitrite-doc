...
You can project the result set using `project()` method. It takes a `Document` as the only input parameter. It returns a `DocumentCursor` object.
...
Now, you want to join these two collections using `userId` field. Then you can do it like this:
